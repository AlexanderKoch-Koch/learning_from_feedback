...
class SingleStepDreamerModel(TorchModelV2, nn.Module):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        if isinstance(self.action_space, gym.spaces.Box):
            self.action_model = Mlp(4, self.obs_size, self.hidden_size * 1, self.action_size)
            self.pred_model = Mlp(2, self.obs_size + self.action_size, self.hidden_size, self.obs_size)
            self.reward_model = Mlp(1, 2 * self.obs_size + self.action_size, self.hidden_size, 1)
        else:
    ...
    def policy(self, obs, state, explore=False):
    ...
    def loss(self, obs, action, reward, valid, done):
        assert obs.shape[0] == action.shape[0] == reward.shape[0] == 2
        if isinstance(self.action_space, gym.spaces.Box):
            action = action[1]
        else:
        ...
        reward_pred = self.reward_model(torch.cat((obs[0], obs_pred.detach(), action), dim=-1))
        ...
        # actor_loss = -reward_pred_on_policy.mean() + 1 * (action ** 2).mean()
        actor_loss = -1 * reward_pred_on_policy.mean() + 4 * ((action.abs() - 0.5).clamp(min=0) ** 2).mean()
    ...
    def get_initial_state(self, batch_size=1, sequence_length=None):
        state = [
            torch.zeros(batch_size, 32, device=self.device),
        ]
        return state
    ...
    def gradient_analysis(self, obs):
        index = 9
        actions = []
        reward_preds = []
        action_grad = []
        for i in range(2000):
            a = torch.torch.tensor([1 - i * 0.001, -0.5], device=self.device, requires_grad=True)
            obs_pred = self.pred_model(torch.cat((obs[0, index], a), dim=-1))
            reward_pred = self.reward_model(torch.cat((obs[0, index], obs_pred, a), dim=-1))
            (reward_pred).backward()
            actions.append(a[0].detach().cpu().numpy())
            reward_preds.append(reward_pred.detach().cpu().numpy())
            action_grad.append(a.grad[0].cpu().numpy())
            # print(
            #     f'a: {a.detach().cpu().numpy()} r:{self.reward_model(torch.cat((obs[0, index], obs_pred.detach(), a), dim=-1)).detach().cpu().numpy()} grad {a.grad.cpu().numpy()}')

        import matplotlib
        matplotlib.use('TkAgg')
        import matplotlib.pyplot as plt

        plt.plot(actions, reward_preds)
        plt.plot(actions, action_grad)
